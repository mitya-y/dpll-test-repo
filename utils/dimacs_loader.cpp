...
std::optional<DimacsFormat> load_cnf(std::string_view filename)
{
  ...
  while (std::getline(file, line))
  {
    ...
    conjunction.disjunctions =
      ...
      std::views::take_while([](int var) { return var != 0; }) |
      std::views::transform([&variables](int var) { variables.insert(std::abs(var)); return var; }) |
...
{
  return other.disjunctions == disjunctions &&
         other.number_of_free_elements == number_of_free_elements &&
         other.satisfied == satisfied;
}

bool DimacsFormat::operator==(const DimacsFormat &other) const
{
  return other.variables == variables &&
